...
public class W_StatBar extends AbstractWidget implements I_Entry {
    ...
    // value: 当前值
    // adder: widget 注册回调
    public W_StatBar(int x, int y, int w, int h, Font font,
    ...
    // ===================== 链式配置 =====================
    // 设置标签文字颜色
    public W_StatBar withLabelColor(int c) {
        labelColor = c;
        return this;
    }

    // 设置数值文字颜色
    public W_StatBar withValueColor(int c) {
        valueColor = c;
        return this;
    }

    // 设置自定义进度条渲染器
    public W_StatBar withBarRenderer(R_Bar r) {
        barRenderer = r;
        return this;
    }

    // 设置最大值（默认 100）
    public W_StatBar withMax(float m) {
        max = m;
        return this;
    }

    // 设置数值格式
    public W_StatBar withFmt(Fmt f) {
        fmt = f;
        return this;
    }

    // 覆盖数值文本（如显示"Lv.5"而非百分比）
    public W_StatBar withValText(String t) {
        valOverride = t;
        return this;
    }
    ...
    protected void extractWidgetRenderState(GuiGraphicsExtractor g, int mx, int my, float pt) {
        ...
        String vs = valOverride != null ? valOverride : switch (fmt) {
            case RATIO ->
                String.format("%.0f%%", value / max * 100);
            case INTEGER ->
                String.format("%.0f/%.0f", value, max);
            case EXP ->
                String.format("%.0f/%.0f", value, max);
        };
